...
## 8. Безопасность `/api/*` (важно — включить, когда виджет начнёт слать ключ)

По умолчанию `AuthMiddleware` **выключен** (`WIDGET_SECURITY_ENFORCE` не задан),
поэтому маршруты `/api/*` сейчас открыты без авторизации. Это осознанно на время
пусконаладки (виджет ещё не прописал заголовок). **Как только виджет реально
шлёт `X-Security-Key`** (тот же секрет прописан и в `.env`, и в настройках
виджета) — обязательно включить защиту:

```bash
# 1) в server/.env
WIDGET_SECRET=<та же случайная строка, что и в настройках виджета>
WIDGET_SECURITY_ENFORCE=true
# 2) применить
docker compose -p raspredelenie -f docker-compose.shared.yml restart app
```

Проверка: запрос к `/api/*` без корректного `X-Security-Key` должен вернуть 401.
`/webhook/leads`, `/health`, `/oauth/callback` ключ не требуют (не блокируются).

> Не включай `enforce=true`, пока ключ не прописан С ОБЕИХ сторон — иначе виджет
> получит 401 на `/api/*` (списки сотрудников/расписаний перестанут грузиться).
